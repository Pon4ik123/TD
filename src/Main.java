...
public class Main {
    public static void main(String[] args) {
        ...
        for(int n = 0; n<=N-1;n++) {
            double t = n / fs;
            double x = Math.sin(2 * Math.PI * fs * t + fi1) * Math.cos(4 * Math.PI * t);
            tab[n] = x;
            System.out.println("x " + n + ": " + x);
        }
        int L = N;
        double[] widmaA = new double[L];
        double[] Atab = new double[L];
        double[] Btab = new double[L];
        for (int k = 0; k <= L-1; k++) {
            double a = 0;
            double b = 0;
            for (int n = 0; n <= L - 1; n++) {
        ...
        XYSeries series = chart.addSeries("Wartości", null, widmaA);
